...
export const StatusCard = ({ result }: StatusCardProps) => {
  ...
  return (
    <div className="relative">
      ...
      <div className={`relative bg-gradient-to-br from-white/90 to-white/60 backdrop-blur-xl rounded-3xl p-8 lg:p-10 border border-white/30 ${getStatusColor(result.status)}`}>
        {/* Header section */}
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-6 mb-8">
          <div className="flex items-center gap-6">
            <div className="flex-shrink-0 text-4xl lg:text-5xl">
              {result.provider.icon}
            </div>
            <div className="min-w-0">
              <h3 className="text-2xl lg:text-3xl font-bold text-slate-800 truncate mb-2">
                {result.provider.name}
              </h3>
              <p className="text-base lg:text-lg text-slate-600 truncate font-mono bg-slate-100 px-4 py-2 rounded-xl">
                {result.model}
              </p>
            </div>
          </div>
          <div className="flex-shrink-0 self-start p-2 bg-white/60 rounded-xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-base lg:text-lg">
          <div className="bg-white/50 p-6 rounded-2xl border border-white/30 hover:bg-white/70 transition-all duration-200">
